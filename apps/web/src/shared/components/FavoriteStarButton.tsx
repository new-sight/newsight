    ...
    fetchFavoriteStocks()
      .then((stocks) => {
        if (active)
          setIsStarred(
            stocks.some(
              (stock) => stock.stockCode.toUpperCase() === symbol.toUpperCase(),
            ),
          );
      })
      .catch(() => undefined);
      ...
      : size === "sm"
        ...
        : size === "md"
          ...
          : size === "lg"
            ...
            : size;
  const numericSize =
    typeof size === "number" ? size : parseFloat(fontSize) || 22;
  return (
    <button
      type="button"
      onClick={handleClick}
      disabled={isLoading}
      className={`${numericSize <= 18 ? "p-1 rounded-lg" : "p-1.5 rounded-xl"} hover:bg-white/10 text-gray-400 hover:text-yellow-400 transition-colors cursor-pointer flex items-center justify-center ${className}`}
      title={isStarred ? "관심 종목 해제" : "관심 종목 추가"}
    >
      <span
        className={`material-symbols-rounded select-none ${isStarred ? "text-yellow-400" : ""}`}
        style={{
          fontSize,
