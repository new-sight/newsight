...
class NewsEmbeddingFilter:
    def __init__(
        ...
        print(
            f"[EmbeddingFilter] Initialized with Ollama Cloud API: {self.api_url}, Model: {self.model_name}"
        )
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            payload_embed = {"model": self.model_name, "input": text}
            response = requests.post(
                url_embed, json=payload_embed, headers=self.headers, timeout=10
            )
            if response.status_code == 200:
                result = response.json()
            ...
            print(
                f"[EmbeddingFilter] Failed to get embedding. HTTP Status: {response.status_code}, Body: {response.text}"
            )
    ...
    def filter_similar_news(
        self, news_items: List[Dict[str, str]], threshold: float = 0.8
    ) -> List[Dict[str, str]]:
        """
        ...
        print(
            f"[EmbeddingFilter] Fetching embeddings from Ollama Cloud for {len(news_items)} items..."
        )
        ...
        for i in range(len(valid_news_items)):
            ...
            for j in range(i + 1, len(valid_news_items)):
                ...
                if similarity >= threshold:
                    print(
                        f"[EmbeddingFilter] Filtering out item {j} ('{valid_news_items[j]['title'][:25]}...') "
        ...
        # 4. 필터링된 고유 뉴스 기사만 반환
        unique_news = [
            valid_news_items[i] for i in range(len(valid_news_items)) if to_keep[i]
        ]
        print(
            f"[EmbeddingFilter] Filtered: {len(news_items)} -> {len(unique_news)} items"
        )
        return unique_news
