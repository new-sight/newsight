...
class NewsExtractor:
    ...
    def extract_metadata(self, title: str, content: str) -> Dict[str, Any]:
        ...
        prompt = (
            ...
            "단, 태그의 이름(name)과 대표 표준명(master)이 영어(English) 및 한국어(Korean)가 아닌 제3국의 언어(예: 중국어, 일본어 등)인 경우에는 반드시 알맞은 한국어 또는 공식 영어명으로 번역해서 응답해줘.\n"
            "특히 상장 기업(Listed Company)의 대표 표준명(master)은 가급적 공식 영어 사명(예: 'Fujitsu', 'Toyota', 'Sony', 'Samsung Electronics', 'TSMC') 또는 숫자형/알파벳 티커 코드(예: '6702', '7203', 'NVDA')로 기재하여 데이터베이스 연동이 원활해지도록 해줘.\n"
